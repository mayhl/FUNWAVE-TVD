...
for i in range(1, numOfETA + 1):
    ...
    plt.axis([-0.5, Lt, min(depth[1, :]) - 0.05, 1])
    ...
    # Bottom Fill:
    plt.fill_between(x, min(depth[1, :]) - 0.05, depth[1, :], where=depth[1, :] > (depth[1, :] - 0.05), facecolor="0.35", hatch="X")

    # Annotations:
    an = plt.annotate("\n~\n|", xy=(WaveMaker, 0.05), xycoords="data", fontsize=20, ha="center", va="center")
    Time = plt.annotate("Time: %4.2f sec" % (i * plotInt - plotInt), xy=(Lt * 0.85, 3), fontsize=16, ha="center", va="center")
