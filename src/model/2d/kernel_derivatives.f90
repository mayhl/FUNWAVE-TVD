module model_kernel_derivatives_mod
   ...
   ! kernels thread over j (pointwise writes, read-only stencils); the
   ! former PURE attributes had to go — the standard bars OMP directives
   ! from pure procedures
...
contains
   ...
   subroutine deriv_x(lp, inv_dx, mask, uin, uout)
   ...
   subroutine deriv_y(lp, inv_dy, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
   ...
   subroutine deriv_xx(lp, inv_dx, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      real(SP), intent(in)  :: inv_dx(:, :), uin(:, :)
      integer, intent(in)  :: mask(:, :)
      real(SP), intent(out) :: uout(:, :)
   ...
   subroutine deriv_yy(lp, inv_dy, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      real(SP), intent(in)  :: inv_dy(:, :), uin(:, :)
      integer, intent(in)  :: mask(:, :)
      real(SP), intent(out) :: uout(:, :)
   ...
   subroutine deriv_xy(lp, inv_dx, inv_dy, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      ...
      integer :: i, j
      real(SP) :: t1, t2
      !$omp parallel do default(shared) schedule(static) private(i, t1, t2)
      do j = lp%jb, lp%je
         do i = lp%ib, lp%ie
   ...
   subroutine deriv_x_high(lp, inv_dx, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      real(SP), intent(in)  :: inv_dx(:, :), uin(:, :)
      integer, intent(in)  :: mask(:, :)
      real(SP), intent(out) :: uout(:, :)
   ...
   subroutine deriv_y_high(lp, inv_dy, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      real(SP), intent(in)  :: inv_dy(:, :), uin(:, :)
      integer, intent(in)  :: mask(:, :)
      real(SP), intent(out) :: uout(:, :)
   ...
   subroutine deriv_xx_high(lp, inv_dx, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      real(SP), intent(in)  :: inv_dx(:, :), uin(:, :)
      integer, intent(in)  :: mask(:, :)
      real(SP), intent(out) :: uout(:, :)
   ...
   subroutine deriv_yy_high(lp, inv_dy, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      real(SP), intent(in)  :: inv_dy(:, :), uin(:, :)
      integer, intent(in)  :: mask(:, :)
      real(SP), intent(out) :: uout(:, :)
   ...
   subroutine deriv_xy_high(lp, inv_dx, inv_dy, mask, uin, uout)
      type(type_loop_bounds), intent(in)  :: lp
      real(SP), intent(in)  :: inv_dx(:, :), inv_dy(:, :), uin(:, :)
      integer, intent(in)  :: mask(:, :)
      real(SP), intent(out) :: uout(:, :)
      integer  :: i, j
      real(SP) :: t1, t2, t3, t4
      !$omp parallel do default(shared) schedule(static) private(i, t1, t2, t3, t4)
      do j = lp%jb, lp%je
         do i = lp%ib, lp%ie
