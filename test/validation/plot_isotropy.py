...
def _run_case(case: Case, verbose: bool) -> float | None:
    ...
    return run_seiche(RUN_ROOT / f"iso_M{case.m}_{case.nx}_{case.ny}",
                      title=f"iso_{case.nx}_{case.ny}", h=H, mglob=case.m,
                      nglob=case.m, dx=DX, dy=DX, mode_x=case.nx, mode_y=case.ny,
                      total_time=total_time, dt_sta=dt_sta, verbose=verbose)
...
def _measure(cases: list[Case], verbose: bool) -> list[Point]:
    ...
    for i, c in enumerate(cases, 1):
        ...
        tag = (f"[{i:2d}/{len(cases)}] M{c.m:3d} ({c.nx},{c.ny})  th={c.theta_deg:5.1f}deg  "
...
_CSV_FIELDS = ["level", "nx", "ny", "k", "kh", "ppw", "theta_deg",
               "C_over_sqrtgh_meas", "C_over_sqrtgh_nwogu", "dev_pct"]
...
def _write_csv(pts: list[Point], path: Path) -> None:
    ...
    with open(path, "w", newline="") as fh:
        ...
        for p in sorted(pts, key=lambda q: (q.level, q.theta_deg)):
            ...
            w.writerow([f"{p.level:.2f}", p.nx, p.ny, f"{p.k:.6f}", f"{p.kh:.6f}",
                        f"{p.ppw:.1f}", f"{p.theta_deg:.3f}", f"{p.y_meas:.6f}",
                        f"{yn:.6f}", f"{(p.y_meas / yn - 1.0) * 100.0:.4f}"])
    print(f"wrote {path}")
...
def _read_csv(path: Path) -> list[Point]:
    with open(path, newline="") as fh:
        return [
            Point(float(r["level"]), int(r["nx"]), int(r["ny"]), float(r["k"]),
                  float(r["kh"]), float(r["ppw"]), float(r["theta_deg"]),
...
def _plot(pts: list[Point], out_png: Path) -> None:
    ...
    fig, (ax0, ax1) = plt.subplots(2, 1, figsize=(9.0, 8.4), sharex=True,
    ...
    for shade, lv in zip(shades, levels):
        ...
        style = dict(marker="o", ms=5, lw=1.3, color=cmap(shade),
    ...
    ax0.legend(loc="best", framealpha=0.95, fontsize=9,
               title="fixed-$|k|$ sets (axis-aligned $\\to$ diagonal)")
...
def main() -> int:
    ...
    print(f"[{tier}] running {len(cases)} oblique-mode cases "
          f"({len({c.level for c in cases})} fixed-|k| levels)\n")
