...
_TIERS = {
    "light": ([12, 100],
              [0.5, 1.5, 3.0, 6.0, 4.0 * math.pi]),
    "medium": ([12, 40, 100],
               [0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0, 10.0, 4.0 * math.pi]),
    "heavy": ([8, 12, 20, 40, 80, 160],
              [0.3, 0.5, 0.7, 0.9, 1.1, 1.3, 1.6, 1.9, 2.2, 2.6, 3.0, 3.5, 4.0,
               4.6, 5.3, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0, 12.0, 4.0 * math.pi]),
}
...
def _run_case(case: Case, verbose: bool) -> float | None:
    ...
    return run_seiche(RUN_ROOT / case.tag, title=case.tag, h=case.h,
                      mglob=case.m, nglob=3, dx=case.dx, dy=DY,
                      mode_x=1, mode_y=0, total_time=total_time, dt_sta=dt_sta,
                      verbose=verbose)
...
_CSV_FIELDS = [
    "M", "ppw", "k", "kh", "h",
    "C_over_sqrtgh_meas", "C_over_sqrtgh_nwogu", "C_over_sqrtgh_airy",
    "model_over_nwogu", "model_over_airy", "err_pct_vs_nwogu",
]
...
def _write_csv(pts: list[Point], path: Path) -> None:
    ...
    with open(path, "w", newline="") as fh:
        ...
        for p in sorted(pts, key=lambda q: (-q.m, q.kh)):
            ...
            w.writerow([
                p.m, f"{p.ppw:.0f}", f"{p.k:.6f}", f"{p.kh:.6f}", f"{p.kh / p.k:.6f}",
                f"{p.y_meas:.6f}", f"{yn:.6f}", f"{ya:.6f}",
                f"{p.y_meas / yn:.6f}", f"{p.y_meas / ya:.6f}",
...
def _read_csv(path: Path) -> list[Point]:
    with open(path, newline="") as fh:
        return [
            Point(int(r["M"]), float(r["k"]), float(r["ppw"]),
                  float(r["kh"]), float(r["C_over_sqrtgh_meas"]))
...
def _plot(pts: list[Point], out_png: Path) -> None:
    ...
    fig, (ax0, ax1, ax2) = plt.subplots(
        3, 1, figsize=(9.0, 11.0), sharex=True,
        gridspec_kw={"height_ratios": [2.2, 1.0, 1.0], "hspace": 0.08},
    )

    ax0.plot(kh, nwogu, "-", color="#1f4e79", lw=2.4, zorder=6,
             label=r"Nwogu (model theory, $\beta_{ref}=-0.531$)")
    ax0.plot(kh, airy, "--", color="#888888", lw=1.8, zorder=2,
             label=r"Airy (exact linear)")
    ...
    for shade, m in zip(shades, groups):
        ...
        style = dict(marker="o", ms=4.5, lw=1.4, color=color,
    ...
    ax2.plot(kh, (nwogu / airy - 1.0) * 100.0, "--", color="#1f4e79", lw=1.8, zorder=5,
    ...
    for x in (math.pi, 2 * math.pi, 3 * math.pi, 4 * math.pi):
        if x <= kh_max:
            ax2.text(x, 0.015, rf"${int(round(x/math.pi))}\pi$",
    ...
    ax1.set_title("numerical error — measured vs the model's own theory (grid convergence)",
                  fontsize=9, loc="left", pad=3)
    ax2.set_ylabel(r"$C_{\rm model}/C_{\rm Airy}-1$  (%)")
    ax2.set_title("departure from exact dispersion (physics + numerics)",
                  fontsize=9, loc="left", pad=3)
...
def main() -> int:
    ...
    ap.add_argument("--csv", type=Path, default=OUT_CSV,
                    help="write the measured sweep table here")
    ap.add_argument("--from-csv", type=Path, default=None,
                    help="skip the sims and re-plot from a saved table")
    ...
    if not BINARY.exists():
        print(f"validation binary not found: {BINARY}\n"
              f"build it first (validation-2d executable).", file=sys.stderr)
        return 1

    tier = "light" if args.quick else args.tier
    cases = _build_cases(tier)
    n_curves = len({c.m for c in cases})
    print(f"[{tier}] running {len(cases)} seiche cases "
          f"({n_curves} resolution curves x {len(cases) // n_curves} kh)\n")
