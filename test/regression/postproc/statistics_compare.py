...
def run(
    ...
    ref_vars = get_output_variables(ref_meta.output_dir, kind="statistics")
    dev_vars = get_output_variables(dev_meta.output_dir, kind="statistics")
    ...
    result = SubsectionResult(kind="statistics", label="Statistics Data", metrics=metrics)
...
def _print_table(ref_meta: RunMetadata, dev_meta: RunMetadata, rows: list[_Row]) -> None:
    ...
    table = Table(
        ...
        show_edge=False,
        pad_edge=True,
        title=f"[bold]Statistics Comparison[/bold]  ·  {meta}",
        title_justify="left",
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python -m test.regression.postproc.statistics_compare <ref_dir> <dev_dir>")
        sys.exit(1)
