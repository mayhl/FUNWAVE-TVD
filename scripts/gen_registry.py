...
def main() -> int:
    ...
    ap.add_argument('--check', action='store_true',
                    help='verify committed output matches the registry')
    args = ap.parse_args()
    ...
    if args.check:
        ...
        if old == new:
            ...
            return 0
        print(f'STALE: {OUTPUT.relative_to(REPO)} does not match registry.yaml — '
              f'rerun: uv run scripts/gen_registry.py', file=sys.stderr)
