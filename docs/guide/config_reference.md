...
| `nu_scale` | `1.0` | — | — | Multiplier on the breaker eddy-viscosity magnitude. Every scheme scales nu with cbrk2 x sqrt(gh) x depth, so cbrk2 carried cessation and magnitude together; nu_scale separates them (1 = as calibrated; the wavemaker-zone term is not scaled). |
