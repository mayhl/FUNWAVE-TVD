...
| `channels.gap` | `0.0` | — | s | Event filter: the condition returning within this many seconds continues the same event (the gap is not counted as duration). 0 = off. |
| `channels.min_duration` | `0.0` | — | s | Event filter: an event shorter than this is discarded at close without a trace. 0 = off. |
